...
public class ServerApp {
  public static void main(String[] args) throws Exception {
    ...
    while (true) {
      ...
      if (command.equalsIgnoreCase("quit")) {
        ...
        out.println(command);
        out.flush(); // 보내는 걸 확실하게 보내고 싶으면 명시적으로 flush메서드 호출
      }
    }

    System.out.println("4) 클라이언트와 접속을 끊음");
    in.close();
    socket.close();

    System.out.println("5) 서버 소켓 종료");
    severSocket.close();        // 더 이상 클라이언트의 접속을 수용하지 않는다.
  ...
  private static void detailBoard(PrintWriter out, BufferedReader in) throws Exception {
    ...
    board.setRegisteredDate(Date.valueOf("2021-09-27"));

    Member m = new Member();
    m.setNo(101);
    m.setName("aaax");
    m.setEmail("aaax@naver.com");
