...
//  =>
//11) promptString, promptInt(), promptDate() 메서드의 코드를 리턴한다
public class App10 {
  ...
  //static String input; 더 이상 프롬포트는 이 변수를 다르지 않음
  ...
  public static void main(String[] args) {
    ...
    while (true) { //->사용자로부터 값을 받는 부분!
      String input = promptString("명령> ");//정확하게 String값 달라
...
static void listMember() {
  ...
  static void addProject() {
    ...
    pNo[pSize] = promptInt("번호? ");//int값을 입력받아줘!
  ...
  static void listProjects () {
    ...
    static Date promptInt (String title) {
      return Date.valueOf(prompt(title)); 
    }
  }
