...
public class Exam0450 {
  ...
  static int sum(int value) { //빙빙도는게 아니라 별도의 변수를 생성하는 것
    if (value == 1)
      return 1; // vlaue값이 1이 되면 value - 1에 리턴됨

    return value + sum(value - 1); // sum에 1을 리턴(마지막에) value + sum(value - 1) 값을 suv(여기에)리턴하는 거
  }

  public static void main(String[] args) {
    // 다음과 같이 작은 수를 계산 할 때는 재귀호출을 사용하는 것이
    // 코드도 간단하고 이해하기도 쉽다.
    System.out.println(sum(5)); // sum(int value)여기 value값에 5 들어감 -> return value + sum(value - 1)돌아가고
    //if 돌아감
  }
}
// JVM Stack 메모리의 사용
